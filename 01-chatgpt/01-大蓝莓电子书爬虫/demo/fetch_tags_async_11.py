        ...
        if portfolio_book_ul is not None:
            ...
            for item in portfolio_items:
                ...
                book_id = re.search(r'/book-content-(\d+)\.html',
                                    link).group(1)
                book_info.append([book_id, title, author, time, f"https://www.dalanmei.com{link}"
                ])

            # tag_name = url.split('-')[2].split('.')[0]  # 提取标签名称
...
async def main():
    ...
    for tag_name, book_info in tag_data.items():
        ...
        sheet.append(["ID", "标题", "作者", "时间", "链接"])  # 添加ID字段
    ...
    # 保存 Excel 文件
    workbook.save('books_async_11.xlsx')

    print("数据提取完成，已保存到 books_async_11.xlsx 文件。")
