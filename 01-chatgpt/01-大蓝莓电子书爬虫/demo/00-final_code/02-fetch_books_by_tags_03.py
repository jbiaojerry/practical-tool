...
async def extract_urls(html_content, session):
...
async def main():
    ...
    url_list = await extract_urls(html_content, session)

    print(f"Step 2: {len(url_list)} URLs generated")

    # 步骤 3：使用异步并发处理 URL 列表
    sem = asyncio.Semaphore(50)  # 适当增加并发数
    async with sem:
        tasks = [process_url(url) for url in url_list]
        results = await asyncio.gather(*tasks)

    print(f"Step 3: {len(results)} URLs processed")
    ...
    # 保存 Excel 文件
    workbook.save('02-fetch_books_by_tags_03.xlsx')
