...
async def main():
    ...
    sem = asyncio.Semaphore(10)  # 限制并发请求数量
    async with sem:
    ...
    # 保存 Excel 文件
    workbook.save('02-fetch_books_by_tags_02.xlsx')

    end_time = time.time()
    print(f"数据提取完成，已保存到 02-fetch_books_by_tags_02.xlsx 文件。执行时间：{end_time - start_time:.2f} 秒")
