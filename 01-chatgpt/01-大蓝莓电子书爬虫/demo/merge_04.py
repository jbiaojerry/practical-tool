        ...
        if len(row) >= 1:
            ...
            if id_field is not None:
                ...
                if data:
                    ...
                    sheet.cell(row=index, column=6, value=epub_link)
                    sheet.cell(row=index, column=7, value=url)


# 保存更新后的books.xlsx文件
books_workbook.save('books_async_04_updated.xlsx')

print("已添加'EPUB下载链接'和'URL'两列，并保存到 books_async_04_updated.xlsx 文件。")
