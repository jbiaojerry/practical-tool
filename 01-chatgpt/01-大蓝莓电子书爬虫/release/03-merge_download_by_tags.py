...
for a_sheet in a_workbook.sheetnames:
    ...
    for row in sheet.iter_rows(min_row=2, values_only=True):
        if len(row) >= 6:
            id_field, _, download_url, mobi_link, epub_link, azw3_link, *_ = row  # 仅提取所需的字段，忽略多余的字段
    ...
    for row in sheet.iter_rows(min_row=1, max_row=1, values_only=True):
        if 'EPUB下载链接' not in row:
            ...
            sheet.cell(row=1, column=8,
                       value='MOBI下载链接')  # 设置第7列的标题为'MOBI下载链接'
            sheet.cell(row=1, column=9,
                       value='AZW3下载链接')  # 设置第8列的标题为'AZW3下载链接'
# 遍历books.xlsx中的所有Sheet
for books_sheet in books_workbook.sheetnames:
    sheet = books_workbook[books_sheet]
...
# 保存更新后的books.xlsx文件
books_workbook.save('03-dalanmei_books_by_tags.xlsx')
